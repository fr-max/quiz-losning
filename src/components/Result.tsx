  ...
  return (
    <div className="px-4 py-6 sm:px-8 sm:py-10 w-full">
      <div className="flex flex-col sm:flex-row sm:items-start sm:gap-10 mb-6">
        ...
        {/* Right: stat + bullets */}
        <div className="sm:w-64 lg:w-80 flex-shrink-0 flex flex-col gap-3">
          {solution.stat && (
            <div className="bg-[#304642] text-white rounded-2xl px-4 py-4">
              <p className="text-xs font-semibold uppercase tracking-widest text-white/50 mb-1">
                Det ser vi typisk hos vores kunder
              </p>
              <p className="text-sm sm:text-base font-semibold leading-snug">{solution.stat}</p>
            </div>
          )}
          <div className="bg-[#304642]/[0.03] border border-[#304642]/10 rounded-2xl px-4 py-4">
            ...
            <ul className="flex flex-col gap-2">
              {solution.bullets.map((bullet, i) => (
                <li key={i} className="flex items-start gap-2.5">
                  ...
                  <span className="text-xs sm:text-sm text-[#304642]">{bullet}</span>
                </li>
              ))}
            </ul>
          </div>
      ...
      {/* Consultant note */}
      <div className="flex items-start gap-3 bg-[#ff5a00]/5 border border-[#ff5a00]/15 rounded-2xl px-4 py-4 mb-5">
        <svg className="mt-0.5 flex-shrink-0" width="16" height="16" viewBox="0 0 18 18" fill="none">
        ...
        <p className="text-xs sm:text-sm text-[#304642]/70 leading-relaxed">
      ...
      <div className="text-center mt-4">
        <button
          onClick={onRestart}
          className="text-xs sm:text-sm text-[#304642]/40 hover:text-[#304642]/70 transition-colors duration-150"
        >
          Tag testen igen
        </button>
      </div>
